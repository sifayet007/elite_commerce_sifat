...
const FeatureOfElideCommerce = () => {
  ...
  return (
    <div className="mt-24 flex flex-col md:flex-row gap-y-10 md:gap-x-5">
      {/* Left Button List */}
      <div className="bg-gradient-to-tl from-[#0CE47F]/10 md:block to-[#01904D]/10 rounded-xl md:flex-1 w-full">
        <h1 className="bg-clip-text text-transparent bg-gradient-to-tl from-[#0CE47F] to-[#01904D] pl-4 pt-4 text-lg md:text-xl">
          All Feature
        </h1>
        <div className="p-4 md:p-8 space-y-3.5">
          {customerFeatures?.map((item, index) => {
            const IconComponent = item.icon;
            return (
              <Button
                key={index}
                className={`py-2 px-5 w-full text-[clamp(12px,2.5vw,16px)] font-medium rounded-full cursor-pointer whitespace-nowrap ${
                  activeIndex === index
          ...
          return (
            <div
              ...
              className={`bg-white relative p-4 sm:p-5 overflow-hidden rounded-xl w-full max-w-[350px] ${
                i === 0 ? "transform -translate-y-3" : "transform translate-y-3"
              }`}
            >
              {/* Shadow circles */}
